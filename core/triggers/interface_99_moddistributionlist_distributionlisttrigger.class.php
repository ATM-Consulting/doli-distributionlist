...
class InterfaceDistributionlisttrigger extends DolibarrTriggers
{
	...
	public function __construct($db)
	{
		...
		$this->family = "demo";
		$this->description = "Distributionlist triggers.";
	...
	{
		if (empty($conf->distributionlist) || empty($conf->distributionlist->enabled)) {
		...
		switch ($action) {
			...
			case 'CONTACT_DELETE':
				...
				//Si le contact est dans une ou plusieurs listes de diffusion, alors, lors de sa suppression, on le décompte dans le  nombre de contacts
				$this->db->begin();

				$error = 0;
				$sql = "SELECT fk_distributionlist FROM ".MAIN_DB_PREFIX."distributionlist_distributionlistsocpeople WHERE fk_socpeople = " .intval($object->id);
				$resql = $this->db->query($sql);
				if($resql){
					while($obj = $this->db->fetch_object($resql)) {
						$distributionList = new DistributionList($this->db);
						$res = $distributionList->fetch($obj->fk_distributionlist);
						if ($res) {
