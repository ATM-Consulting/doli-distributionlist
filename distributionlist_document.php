...
/**
 *  \file       distributionlist_document.php
 *  \ingroup    distributionlist
 *  \brief      Tab for documents linked to DistributionList
 */
...
$hookmanager->initHooks(array('distributionlistdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
//if ($id > 0 || ! empty($ref)) $upload_dir = $conf->distributionlist->multidir_output[$object->entity?$object->entity:$conf->entity] . "/distributionlist/" . dol_sanitizeFileName($object->id);
if ($id > 0 || !empty($ref)) $upload_dir = $conf->distributionlist->multidir_output[$object->entity ? $object->entity : $conf->entity]."/distributionlist/".dol_sanitizeFileName($object->ref);
...
$title = $langs->trans("DistributionList").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("DistributionList"), -1, $object->picto);
	...
	$relativepathwithnofile = 'distributionlist/'.dol_sanitizeFileName($object->ref).'/';
